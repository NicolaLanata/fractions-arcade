...
(function () {
  ...
  const GAME_ID_TO_QCOUNT = {};
  const RECORD_KEY_TO_GAME_ID = {};
  const RECORD_KEY_TO_QCOUNT = {};
  const STAR_GAME_IDS = new Set();

  FRACTIONS_ARCADE_GAMES.forEach((g) => {
    if (!g || typeof g !== "object") return;
    if (typeof g.id === "string" && g.id) {
      GAME_ID_TO_QCOUNT[g.id] = Number.isFinite(g.qCount) ? g.qCount : null;
      if (Number.isFinite(g.qCount) && g.qCount > 0 && typeof g.bestKey === "string" && g.bestKey) {
        STAR_GAME_IDS.add(g.id);
      }
    }
    ...
    Object.keys(base.users).forEach((rawId) => {
      ...
      Object.keys(games).forEach((gid) => {
        ...
        normalizedGames[gid] = {
          ...
          stars: Number.isFinite(g.stars) ? Math.max(0, Math.min(5, Math.floor(g.stars))) : 0,
  ...
  function parseFractionsBest(raw, qCount) {
    ...
    if (hasGreen && Number.isFinite(timeMs)) {
      ...
      if (Number.isFinite(qCount) && qCount > 0) {
        ...
        return {
          stars: Math.max(0, Math.min(5, Math.floor(g))),
          scoreValue: g,
          scoreLabel: `${g}/${qCount}`,
          bestCorrect: g,
          bestTotal: qCount,
  ...
  function patchGameProgress(gameId, patch) {
    ...
    if (Number.isFinite(patch.bestCorrect) && Number.isFinite(patch.bestTimeMs)) {
      ...
      if (betterByScore || (tiedScore && betterByTime)) {
        ...
        if (STAR_GAME_IDS.has(gameId)) {
          const maxStars = Number.isFinite(GAME_ID_TO_QCOUNT[gameId]) && GAME_ID_TO_QCOUNT[gameId] > 0
            ? Math.min(5, Math.floor(GAME_ID_TO_QCOUNT[gameId]))
            : 5;
          game.stars = Math.max(0, Math.min(maxStars, Math.floor(patch.bestCorrect)));
        }
      }
    }
  ...
  function gameSupportsStars(gameMeta) {
    return !!(gameMeta && gameMeta.id && STAR_GAME_IDS.has(gameMeta.id));
  }

  function gameMaxStars(gameId) {
    const q = GAME_ID_TO_QCOUNT[gameId];
    if (Number.isFinite(q) && q > 0) return Math.min(5, Math.floor(q));
    return 5;
  }

  function earnedStarsForGameState(gameId, gameState) {
    if (!STAR_GAME_IDS.has(gameId)) return 0;
    const max = gameMaxStars(gameId);

    // Derive stars from best greens so stale stored star values can't undercount.
    if (gameState && Number.isFinite(gameState.bestCorrect)) {
      return Math.max(0, Math.min(max, Math.floor(gameState.bestCorrect)));
    }

    if (gameState && Number.isFinite(gameState.stars)) {
      return Math.max(0, Math.min(max, Math.floor(gameState.stars)));
    }

    return 0;
  }

  function gameStarsLine(gameMeta, gameState) {
    if (!gameSupportsStars(gameMeta)) return "";
    const max = gameMaxStars(gameMeta.id);
    const stars = earnedStarsForGameState(gameMeta.id, gameState);
    return `${stars}/${max}`;
  }

  function countUserStats(user, gameIds) {
    const maxStars = gameIds
      .filter((id) => STAR_GAME_IDS.has(id))
      .reduce((sum, id) => sum + gameMaxStars(id), 0);
    ...
    gameIds.forEach((id) => {
      ...
      if (STAR_GAME_IDS.has(id)) {
        const earned = earnedStarsForGameState(id, g);
        stars += earned;
      }
  ...
  function renderHome() {
      ...
      host.innerHTML = `
        <section class="arcade-adventure-card" aria-label="Adventure HQ">
          ...
          <div class="arcade-summary-grid">
            ...
            <div class="arcade-summary-item"><div class="k">Stars</div><div class="v">0/${stats.maxStars}</div></div>
    ...
    const recordRows = gameIds
      .map((id) => {
        ...
        const starsLine = gameStarsLine(meta, g);
        const starsHtml = starsLine
          ? `<div class="arcade-record-stars">Stars ${escapeHtml(starsLine)}</div>`
          : "";
        return `
    ...
    host.innerHTML = `
      <section class="arcade-adventure-card" aria-label="Adventure HQ">
        ...
        <div class="arcade-summary-grid">
          ...
          <div class="arcade-summary-item"><div class="k">Stars</div><div class="v">${stats.stars}/${stats.maxStars}</div></div>
      ...
      <div class="arcade-record-modal" id="recordModal" hidden>
        <div class="arcade-record-dialog">
          ...
          <div class="arcade-record-summary">
            ...
            <div class="arcade-record-pill">Stars ${stats.stars}/${stats.maxStars}</div>
    ...
    function tileHtml(g) {
      ...
      const starsLine = gameStarsLine(g, s);
      const starsHtml = starsLine
        ? `<div class="tile-stars">Stars ${escapeHtml(starsLine)}</div>`
        : "";
